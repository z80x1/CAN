function ms0=calcproperties(ms0,moltype,fl_only_sdesc)
%calculate molecule proporties
%calcproperties(ms0,moltype,fl_only_sdesc)
%ms0 - volecule structure to analyze
%moltype - type of molecule
%fl_only_sdesc - calculate only prop.sdesc field
%
% Version 1.0    
...
%    end
    ...
    if any(moltype==[413]) %AZT
      iN31= ms0.ind(find(find(strcmp(pind.labels,'pN31'))==ms0.pind)); 
      iN32= ms0.ind(find(find(strcmp(pind.labels,'pN32'))==ms0.pind)); 
    end
%--------------------------------------------------------------------
...
%       prop.tdelta_old=torang(ms0,iO4,iC4,iC3,iO3); %errorous definition as in Seanger (Mir 1984, page 29)
%     end
    if (moltype==413) %Zidovudine (AZT)
      prop.tdelta=torang(ms0,iC5,iC4,iC3,iN31); 
    elseif (moltype>100 && mod(moltype,100)==12) %dideoxy molecules
      ; 
    elseif ~(isempty(iC5) || isempty(iC4) || isempty(iC3) || isempty(iO3))
      prop.tdelta=torang(ms0,iC5,iC4,iC3,iO3); 
    else
      error('error: No delta torsion found');
    end
    ...
    if ~(moltype>100 && mod(moltype,100)==12) %non dideoxy molecules
        ...
        elseif (moltype==413) %Zidovudine (AZT)
          prop.tepsilon=torang(ms0,iC4,iC3,iN31,iN32);
          prop.tkappa2=torang(ms0,iH31,iC3,iN31,iN32);
...
%      prop.HbondVMind=0;
    ...
    if ~( (moltype>100 && any(mod(moltype,100)==[31,12])) || any(moltype==[6,23,30,413]) ) %not for 3'nucleotides and AZT
...
%       prop.dplC3=d(3); %distance from least square fit ring plane to atom C3'
    ...
    %determining sugar ring conformation only by P angle value
      ...
      if (moltype==413) %Zidovudine (AZT)
          prop.hta4=vecangd(ms0,iN31,iN32,iC3,iH31); 
	  elseif (moltype>100 && mod(moltype,100)==12) %dideoxy molecules
      	  ; % hta4 doesn't exist
      elseif ~(isempty(iO3) || isempty(iP3) || isempty(iC3) || isempty(iH31))
      ...
      elseif ~(isempty(iO3) || isempty(iH32) || isempty(iC3) || isempty(iH31))
          ...
          error('error: No hta4 torsion find');
      end
    ...
    if ~any(moltype==[2,3,4,5,6,7,8]) %now this block is for all
        ...
        if moltype<100
            ...
            if any(floor(moltype/100)==[2,4,5]) %Cyd, Urd, Thd
                if any(mod(moltype,100)==[30,31,32,40,50,51,52,61,70,11,12,13,21,22,23,24,9]) 
                    aindx=[ibN1,ibC2,ibN3,ibC4,ibC5,ibC6]; %indexes of atoms in aminobase plane
...
%                    aindx=[ibN1,ibC2,ibN3,ibC4,ibBr5,ibC6];
            ...
            elseif any(floor(moltype/100)==[1,3]) %Ade, Guo
                if any(mod(moltype,100)==[30,31,40,50,51,52,53,61,70,11,12,90,91,9])
                    aindx =[ibN1,ibC2,ibN3,ibC4,ibC5,ibC6];
                    aindx2=[ibC4,ibC5,ibN7,ibC8,ibN9];
    ...
    %N1bplane - least square plane of purine ring
      ...
      %    [x0,N1bplane,d,normd]=lsplane([ms0.x(aindx) ms0.y(aindx) ms0.z(aindx)]);
        ...
        elseif moltype==16 || ( floor(moltype/100)==3 && moltype~=390 && moltype~=391 ) % for Guo molecules only
...
%          sgn=sign(dot(cross(createvect(ms0,aindx(1),ms0,aindx(3)),createvect(ms0,aindx(1),ms0,aindx(5))),N2)); %distinguish cases when bond is under plane or above it
        ...
        elseif moltype==15 || floor(moltype/100)==1 % for Ado molecules only
...
%        torbpl(1)=torang(ms0,aindx(6),aindx(1),aindx(2),aindx(3)); %torsion angle N6N1C2N3 in aminobase
    ...
    if ~any(moltype==30)    
      prop.sdesc=GLsdescr(ceil(prop.Pdeg/36)); %P
      ...
      if isfield(prop,'hta5')
        prop.sdesc=[prop.sdesc GLsdesc(ceil(prop.hta5/120))]; %chi
      end
