...
%identifies molecule structure. Returns modified struct ms0 with pind 
...
%fl_restart : flag to continue identification
%
% Version 1.0    
...
%100913 added nfrag field for many links molecules
...
%moltype=8: 1'2'deoxyribose                     (r8)  200608 done
...
%moltype=14: 6azacytidine           6AC         (re)  yurenko??
...
%moltype=109: dAdo N1 H+                        (r109)
%moltype=110: 2'deoxy-8aza-adenosine     d8AA    (r110) 	2do
%moltype=111: 2'deoxy-5'tioadenosine            (r111)
%moltype=112: 2',3'-dideoxy-didehydro-adenosine DDA, d4A (r112)  yurenko
%moltype=130: dAMP 5'                           (r130)
%moltype=131: dAMP 3'                           (r131)
%moltype=140: rAdo                              (r140)	done
%moltype=150: dAdo imino                        (r150)
%moltype=151: 2'-deoxy-2aminopurine             (r151)       done
%moltype=152: 2'deoxy-8oxo-adenosine     d8OA   (r152)		katarina
%moltype=161: 2'deoxy-purine             dP     (r161)       done
%moltype=162: 2'-deoxy-8-aza-purine      d8AP   (r162)       done


%moltype=201: Cytidine                          (r201)
%moltype=209: dCyd N3 H+                        (r209)
%moltype=211: 2'deoxy-5'tiocytidine             (r211)
%moltype=212: 2',3'-dideoxy-didehydro-cytidine, d4C	  (r212) yurenko
%moltype=220: 2'deoxy-6flourinecytidine d6FC    (r220)
%moltype=230: dCMP 5'                           (r230)
%moltype=231: dCMP 3'                           (r231)
%moltype=232:??? 2009-12
%moltype=240: rCyd                              (r240)	done
%moltype=241: 6AC N3 protonated                 (r241)
%moltype=250: dCyd imino C4                     (r250)
%incorrect molecule!!! %moltype=251: dCyd C6 imino (r251)
%moltype=270: 2'-deoxy-5-methylcytidine dm5Cyd  (r270)           2do


%moltype=301: Guanine                           (r301)
%moltype=309: dGuo N1 H-                        (r309)
%moltype=310: 2'deoxy-8azaguanosine     d8AG    (r310)	2do
%moltype=311: 2'deoxy-5'tioguanosine            (r311)
%moltype=312: 2',3'-dideoxy-didehydro-guanosine DDG, d4G (r312) yurenko
%moltype=330: dGMP 5'                           (r330)
%moltype=331: dGMP 3'                           (r331)
%moltype=340: rGuo                              (r340)	done
%moltype=350: dGuo lactim                       (r350)
%moltype=352: 2'deoxy-8oxo-guanosine     d8OG   (r352)  2do
%moltype=353: 2'deoxy-7methyl-guanosine  dm7G   (r353)  2do
%moltype=390: 2'deoxy-xanthosine    dXao   dX     (r390)	2do
%moltype=391: 2'deoxy-inosine,        dIno dI     (r391)	2do (base: hypoxanthine, 6-юъёшяєЁшэ)

%moltype=401: Thymine                           (r401)
%moltype=402: Thymine crystal cell              (r402)
%moltype=409: dThd N3 H-                        (r409)
%moltype=411: 2'deoxy-5'tiothymidine            (r411)
%moltype=412: 2',3'-dideoxy-didehydro-thymidine d4T (r412) allien/yurenko
%moltype=413: Zidovudine (INN) or azidothymidine (AZT) (also called ZDV) nastya
...
%moltype=512: 2',3'-dideoxy-uridine, d4U        (r512)	yurenko
...
%moltype=523: 2'deoxy-5chlorouridine  d5ClU     (r523)  2do
%moltype=524: 2'deoxy-5fluorineuridine  d5FU    (r524)	2do
%moltype=530: dUMP 5'                           (r530)
%moltype=531: dUMP 3'                           (r531)
%moltype=540: rUrd                              (r540)	done
%moltype=550: dUrd lactim                       (r550)
%moltype=554: 6,6'O-anhydro-6,6'-dihydroxy-2'-deoxyuridine cU (r554)
...
%     if ~iC4, lastwarn('error: identmol: C4 not found'); return, end
    ...
    if any(moltype~=6) && ~(moltype>100 && any(mod(moltype,100)==[12,13])) %3'deoxy molecules
...
%       (moltype>100 && any(mod(moltype,100)==[10,20,30,31,50])) %in 1'2'deoxyribose H22 is bonded to C2
    ...
    elseif moltype==6  %3'deoxy molecules
        ...
        ms0.marked(iH32)=1;
    elseif (moltype>100 && mod(moltype,100)==13) %azidothymidine
        iN31 = identatom(ms0,'N',iC3);
        ms0.pind(iN31) = find(strcmp(pind.labels,'pN31'));
        ms0.marked(iN31)=1;
        iN32 = identatom(ms0,'N',iN31);
        ms0.pind(iN32) = find(strcmp(pind.labels,'pN32'));
        ms0.marked(iN32)=1;
        iN33 = identatom(ms0,'N',iN32);
        ms0.pind(iN33) = find(strcmp(pind.labels,'pN33'));
        ms0.marked(iN33)=1;
    else
    ...
    if (moltype>100 && any(mod(moltype,100)==[30])) || moltype==22 %5'nucleotides
        ms0=detectPchain(ms0,iO5,'pP5');
    elseif (moltype>100 && any(mod(moltype,100)==[32])) %5'nucleotides w/o NH3
        ms0=detectPchain(ms0,iO5,'pP5',0);
    elseif any(moltype==[910,920,950]) %#ok

        iP = identatom(ms0,'P',iO5);
        if ~iP, lastwarn('error: identmol: iP not found');return, end
        ms0.pind(iP) = find(strcmp(pind.labels,'pP5'));
        ms0.marked(iP)=1;
        ms0.nfrag(iP)=uint8(0); %gator110116
       ...
       if  any(moltype==[920]) 
        iHP2 = identatom(ms0,'H',iOP2);        
       if ~iHP2, lastwarn('error: identmol: iHP2 not found');return, end
       ms0.pind(iHP2) = find(strcmp(pind.labels,'pHP2'));
        ms0.marked(iHP2)=1;
        ms0.nfrag(iHP2)=uint8(0); %gator110116
       end 
       
       if  any(moltype==[950]) 
         iNa = strcmpcellar(ms0.labels,'Na');  
 %         iNa = identatom(ms0,'Na',iOP1);        
         if ~iNa, ('error: identmol: iNa not found');return, end
         ms0.pind(iNa) = find(strcmp(pind.labels,'pNa'));
         ms0.markelastwarnd(iNa)=1;
         ms0.nfrag(iNa)=uint8(0); %gator110116
       end 
...
if any(moltype==[2,3,4,5,6,7,8,22,23,910,920,950]) %sugars
...
%        ms0.marked(ibNC7)=1;
 ...
 %       ms0.marked(ibH3)=1;
    ...
    if all(moltype~=[350,353,309]) %dGuo lactim, 7-methyl, dGuo N1 H-
        ibH1 = identatom(ms0,'H',ibN1);
        ms0.pind(ibH1) = find(strcmp(pind.labels,'bH1'));
        ms0.marked(ibH1)=1;
    end
...
%    ms0.marked(ibH53)=1;
    ...
    if all(moltype~=[450,409]) %dThd lactim, dThd N3 H-
        ibH3 = identatom(ms0,'H',ibN3);
        ms0.pind(ibH3) = find(strcmp(pind.labels,'bH3'));
        ms0.marked(ibH3)=1;
    end
...
elseif any(moltype==[12,17,19]) || (moltype>=500 && moltype <= 599)%uridine
    ...
    if moltype==19 %5aza
        ...
        if any(moltype==[521,522]) %5Br Urd
            ...
            ms0.marked(ibBr5)=1;
        elseif any(moltype==[523]) %#ok %5Cl Urd 
            ibCl5 = identatom(ms0,'Cl',ibC5);
            ms0.pind(ibCl5) = find(strcmp(pind.labels,'bCl5'));
            ms0.marked(ibCl5)=1;
        elseif any(moltype==[524]) %#ok %5F Urd 
            ibF5 = identatom(ms0,'F',ibC5);
            ms0.pind(ibF5) = find(strcmp(pind.labels,'bF5'));
            ms0.marked(ibF5)=1;
        else
            ibH5 = identatom(ms0,'H',ibC5);
            ms0.pind(ibH5) = find(strcmp(pind.labels,'bH5'));
...
ms0.intort  
  ...
  function ms0=detectPchain(ms0,iOatom,Patomstring, withHN3)
...
%    global CH
    ...
    if withHN3
        ...
        if ~ixN, error('error: identmol: nucleotide analysis specified but xN atom is not found');return, end
