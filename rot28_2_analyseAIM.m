...
fl_print_only_canonical_bonds = 0;
...
csvfile=[CD.xlsdir filesep xlsfile '.csv'] %#ok
...
elseif isfield(workdb(1).gaussian,'MP2_6311__Gdp')
    ...
    energyfield='dummy';
    warning('energy field not found');
end
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  recnum=6509
%  strdisp=['!!!Attention recnum=', int2str(recnum) , '!!!!'];
%  disp(strdisp);
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         for k=1:numel(AIMdesc)
%          AIM.desc(end+1) =  AIMdesc(k);
%         end 
         AIM.confind(hbondind:hbondind+hbondnumi-1) = i;
         AIM.ro(hbondind:hbondind+hbondnumi-1) = ms0.AIM.ro;
          AIM.DelSqRho(hbondind:hbondind+hbondnumi-1) = ms0.AIM.DelSqRho;
...
AIM.upinds = (98*(AIM.nfrag==2)+AIM.nfrag).*AIM.pinds; % ����� ���������� ������� - � ������ ���������
...
if size(uniquepinds,2)==4
    HBw4atoms=find(uniquepinds(:,4)~=0); % ������� ����� �� ������� �� 4-� ������
else
    HBw4atoms=[];
end
...
for i=1:size(Hbondtype.pinds,1) %cycle over all Hbond's types
    ...
    for j=1:numel(curinds) %over all bond of specific type %���� �� ���� ������ ������������� ����
      curconf=AIM.confind(curinds(j)); %index of current conformer
      ms0 = workdb(curconf);

      if (~isfield(ms0,'nfrag')) %zhr110310
          ms0.nfrag = ones(size(ms0.ind));
      else
          ms0.nfrag(ms0.nfrag==0)=1;
      end

      if any(Hbondtype.nfrags)
...
%       end
      ...
      if (moltype==7 || moltype==8 || any(moltype==[910,920,950]) )
      else
    ...
    if any(moltype==[910,920,950])
      ...
      if ( ~isfield(ms0.prop,'tdelta') && isfield(ms0.prop,'delta_2') )
	      ms0.prop.tdelta=ms0.prop.delta_2;
      end
      ...
      if (~isfield(ms0.prop,'tepsilon') && isfield(ms0.prop,'epsilon_2') )
	      ms0.prop.tepsilon=ms0.prop.epsilon_2;
      end
  ...
  %if i==7 , keyboard, end
    ...
    Hbondtype.avgEhb(i) = mean(abs(AIM.V(curinds))*0.5*CC.encoef);
%    HbondVMfreq(curinds,Hbond_bH6_ind(curinds))

    if size(Hbondtype.pinds,2)==4 && Hbondtype.pinds(i,4)
        Hbondtype.title(i)={[pind.labels{Hbondtype.pinds(i,1)} pind.labels{Hbondtype.pinds(i,2)} '...' pind.labels{Hbondtype.pinds(i,3)} pind.labels{Hbondtype.pinds(i,4)} ' (' int2str(Hbondtype.nfrags(i,1)) '-' int2str(Hbondtype.nfrags(i,4)) ')']};
    elseif Hbondtype.pinds(i,3)
        ...
        Hbondtype.title(i)={[pind.labels{Hbondtype.pinds(i,1)}  '...' pind.labels{Hbondtype.pinds(i,2)} ' (' int2str(Hbondtype.nfrags(i,1)) '-' int2str(Hbondtype.nfrags(i,2)) ')']};
    end
...
if flwritefile
  ...
  if 0 && isunix %nor working 
     
        File=fullfile(pwd,xlsfile);
        Hbondtype2exportdesc =  [ 
                    {'H-bond name'}; {'number'};...
                    ...
                    {'minEhb, kcal/mol'}; {'maxEhb, kcal/mol'}; {'avgEhb, kcal/mol'};...
                    {'conf_syn'}; {'conf_anti'}; 
                    {'conf_north'}; {'conf_south'};...
        ...
        if (moltype>100 && mod(moltype,100)==40) || moltype==7
            Hbondtype2exportdesc =  [ Hbondtype2exportdesc; 
                    {'conf_etagp'}; {'conf_etat'}; {'conf_etagm'};...
        ...
        xlswrite1spec(File,Hbondtype.avgEhb,'AIMavgdata',['B' int2str(l)],params); l=l+1;
    ...
    if any(moltype==[910,920,950])
        Hbondtype2exportdesc =  [ 
                    ...
                    {'conf_deltagp'}; {'conf_deltat'}; {'conf_deltagm'};...
                    {'conf_epsilongp'}; {'conf_epsilont'}; {'conf_epsilongm'};...
                    {'conf_alphagp'};{'conf_alphat'};{'conf_alphagm'};...
                    {'conf_zetagp'};{'conf_zetat'};{'conf_zetagm'};
                    ];
    else    
        Hbondtype2exportdesc =  [ 
                    {'H-bond name'}; {'number'};...
...
%----------------------------------------
    ...
    if flwrite_bondlist %zhr091208
      ...
      str=sprintf('%s\t\t%s\t\t%s\t\t%s\t\t%s\t\t%s\t\t%s\t\t%s\t\t%s',...
                  'sdesc','bond','rho','DelSqRho','V','G','K','L','E_{HB}','ABdist','HBdist','AHBang','BondEl');
      disp(str);
      allHbondsdata = cell(0,13);

      numconf_by_Hbondsnum = zeros(1,10);
      numconf_syn_by_Hbondsnum = zeros(1,10);
      numconf_anti_by_Hbondsnum = zeros(1,10);
      for i=1:recnum

          ms0 =  workdb(i);
		  Hbondsnum = 0;

    %       if all(ms0.prop.sdesc=='aAbababAaab') 
    %         ms0.AIM=[]
    %          disp('!!! ms0.AIM=[] !!!');
    %       end
          ...
          if ~isempty(ms0.AIM) 
              ...
              for j=1:numel(ms0.AIM.desc)

                pinds_count = sum(ms0.AIM.pinds(j,:)~=0);

	            if (pinds_count>=3)
					Hbondsnum = Hbondsnum + 1;
	            end
                ...
                if fl_print_only_canonical_bonds==0 || pinds_count==3
                    ...
                    if 0 
                        ...
                        if (~isfield(ms0,'nfrag')) %zhr110311
                            ms0.nfrag = ones(size(ms0.ind));
                    ...
                    for k=1:numel(ms0.AIM.desc) 

                       atom1ind = find(((ms0.nfrag==ms0.AIM.nfrag(k,1))) & (ms0.pind==ms0.AIM.pinds(k,1)));
                       atom2ind = find(((ms0.nfrag==ms0.AIM.nfrag(k,2))) & (ms0.pind==ms0.AIM.pinds(k,2)));
                       if numel(ms0.AIM.nfrag)>2
                           atom3ind = find(((ms0.nfrag==ms0.AIM.nfrag(k,3))) & (ms0.pind==ms0.AIM.pinds(k,3)));
                       end

                       if isempty(atom1ind)
                         error([ms0.prop.sdesc ': No atom ' pind.labels{ms0.AIM.pinds(k,1)} ' exists in current molecule']);
                       end
                       if isempty(atom2ind)
                         error([ms0.prop.sdesc ': No atom ' pind.labels{ms0.AIM.pinds(k,2)} ' exists in current molecule']);
                       end
                 %       if isempty(atom3ind)
                 %         error([ms0.prop.sdesc ': No atom ' pind.labels{Hbondtype.pinds(i,3)} ' exists in current molecule']);
                 %       end
                       ...
                       if ~isempty(atom3ind)
                           ms0.AIM.ABdist(k)=adist(ms0, atom1ind, atom3ind);
                           ms0.AIM.HBdist(k)=adist(ms0, atom2ind, atom3ind);
                           ms0.AIM.AHBang(k)=valang(ms0, atom1ind, atom2ind, atom3ind);
                       else
                           ms0.AIM.ABdist(k)=0;
                           ms0.AIM.HBdist(k)=adist(ms0,atom1ind,atom2ind);
                           ms0.AIM.AHBang(k)=0;
                       end
                    end
                    ...
                    ABdist    = ms0.AIM.ABdist(j);
                    HBdist    = ms0.AIM.HBdist(j);
                    AHBang    = ms0.AIM.AHBang (j);
                    BondEl    = ms0.AIM.BondEl (j);

                    str=sprintf('%s\t%s\t%7.3f\t%7.3f\t%7.3g\t%7.3g\t%7.3g\t%7.3g\t%7.2f%7.3g\t%7.4g\t%7.4g\t%7.4g',...
                                ms0.prop.sdesc,bondstr,ro,DelSqRho,V,G,K,L,E_HB,ABdist,HBdist,AHBang,BondEl);
                    disp(str);
    % if ~(pinds_count==3)
    %     disp(str);
    % end    

                    allHbondsdata(end+1,:) = [{ms0.prop.sdesc},{bondstr},{ro},{DelSqRho},{V},{G},{K},{L},{E_HB},{ABdist},{HBdist},{AHBang},{BondEl*100}];
                end
              end


          end
          numconf_by_Hbondsnum(Hbondsnum+1) = numconf_by_Hbondsnum(Hbondsnum+1) + 1;
          if ms0.prop.sdesc(end)=='T' || ms0.prop.sdesc(end)=='S' 
              numconf_syn_by_Hbondsnum(Hbondsnum+1) = numconf_syn_by_Hbondsnum(Hbondsnum+1) + 1;
          end
          if ms0.prop.sdesc(end)=='B' || ms0.prop.sdesc(end)=='C' 
              numconf_anti_by_Hbondsnum(Hbondsnum+1) = numconf_anti_by_Hbondsnum(Hbondsnum+1) + 1;
          end
      end

      numconf_by_Hbondsnum
      numconf_syn_by_Hbondsnum
      numconf_anti_by_Hbondsnum
    end
  ...
  try
    ...
    params.ColorIndex = 1;
    params.NumberFormat = '@';
    xlswrite1spec(File,[{'sdesc'},{'bond'},{'rho'},{'DelSqRho'},{'V'},{'G'},{'K'},{'L'},{'E_HB'},{'ABdist'},{'HBdist'},{'AHBang'},{'BondEl*100'}],'allHbondsdata',['A1'],params);

    xlswrite1spec(File,allHbondsdata(:,1),'allHbondsdata',['A2'],params);
    params.Bold = 0;
    xlswrite1spec(File,allHbondsdata(:,2),'allHbondsdata',['B2'],params);
    params.NumberFormat = '0.000';
    xlswrite1spec(File,allHbondsdata(:,3:11),'allHbondsdata',['C2'],params);
    params.NumberFormat = '0.0';
    xlswrite1spec(File,allHbondsdata(:,12),'allHbondsdata',['L2'],params);
    params.NumberFormat = '0';
    xlswrite1spec(File,allHbondsdata(:,13),'allHbondsdata',['M2'],params);
