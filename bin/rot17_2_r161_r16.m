%change r161 (deoxypurine) ro r16 (dGuo) 
%by changing H6 to bCO6, adding bH1, changing bH2 to bCN2 and adding bH21,bH22 atoms 
%
% Version 1.0    
...
for i=ind
    ...
    %changing bH6 to bCO6. C-O distance equals 1.22A
    inde=find(ms0.pind==find(strcmp(pind.labels,'bH6')));
    ms0.pind(inde) = find(strcmp(pind.labels,'bCO6'));
    ms0.labels(inde)={'O'};
    
    distCO=1.22; %CO distance is equal 1.22
    ...
    ef = e-f;
    e=f+ef/norm(ef)*distCO;
    
    ms0.x(inde)=e(1);
    ms0.y(inde)=e(2);
    ms0.z(inde)=e(3);
    
    %changing bH2 to bCN6. C-O distance equals 1.22A
    inde=find(ms0.pind==find(strcmp(pind.labels,'bH2')));
    ms0.pind(inde) = find(strcmp(pind.labels,'bCN2'));
    ms0.labels(inde)={'N'};
    
    distCN=1.38; %CN distance is equal 1.38
    ...
    %adding H21 and H22
    indA=find(ms0.pind==find(strcmp(pind.labels,'bN3')));
    indB=find(ms0.pind==find(strcmp(pind.labels,'bC2')));
    indC=find(ms0.pind==find(strcmp(pind.labels,'bN1')));
    ...
    indB=find(ms0.pind==find(strcmp(pind.labels,'bN1')));
    indC=find(ms0.pind==find(strcmp(pind.labels,'bC2')));
    A=[ms0.x(indA) ms0.y(indA) ms0.z(indA)];
    B=[ms0.x(indB) ms0.y(indB) ms0.z(indB)];
    C=[ms0.x(indC) ms0.y(indC) ms0.z(indC)];
    X=(A+C)./2;
    ...
    ms0.labels(end+1)={'H'};
    ms0.ind(end+1)=max(ms0.ind)+1;
    ms0.pind(end+1)=find(strcmp(pind.labels,'bH1'));
