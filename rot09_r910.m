...
fl_test = 1 %#ok set to 1 to preventing loading of DB file
%--------- data section -------------------------------
...
ms1 = workdb(1);
if (moltype>=900 && moltype<=999)
    if (fl_test)
        ms1 = createbondtable(ms1);
        [ms1,status] = identmol(ms1,moltype);
    end
    ms1 = calcproperties910(ms1,moltype);
else
    ms1 = calcproperties(ms1,moltype);
end

%--distance & angles & torsion matrixes
%[pindsorted,pindsortii]=sort(pind.labels);
[distmatrorder,distmatrordii] = sortrows(ms1.prop.distmatrpinds);
distmatrdesc=cell(size(distmatrordii));
ind=0;
for ii=distmatrordii'
  ind=ind+1;
  distmatrdesc(ind)={[pind.labels{ms1.prop.distmatrpinds(ii,1)}, pind.labels{ms1.prop.distmatrpinds(ii,2)}]};
end
[anglematrorder,anglematrordii] = sortrows(ms1.prop.anglematrpinds);
anglematrdesc=cell(size(anglematrordii));
ind=0;
for ii=anglematrordii'
  ind=ind+1;
  anglematrdesc(ind)={[pind.labels{ms1.prop.anglematrpinds(ii,1)} pind.labels{ms1.prop.anglematrpinds(ii,2)}...
               pind.labels{ms1.prop.anglematrpinds(ii,3)}]};
end
[tormatrorder,tormatrordii] = sortrows(ms1.prop.tormatrpinds);
tormatrdesc=cell(size(tormatrordii));
ind=0;
for ii=tormatrordii'
  ind=ind+1;
  tormatrdesc(ind)={[pind.labels{ms1.prop.tormatrpinds(ii,1)} pind.labels{ms1.prop.tormatrpinds(ii,2)} ...
             pind.labels{ms1.prop.tormatrpinds(ii,3)} pind.labels{ms1.prop.tormatrpinds(ii,4)}]};
end
%--end
...
[Hind,Hbondpindlabels,HbondLMind,HbondVMind,HbondVMintensity,HbondLMfreq,HbondVMfreq] = ...
    deal(repmat(0,recnum,numel(strcmpcellar(ms1.labels,'H'))));
Hbondpindlabels = repmat({},numel(strcmpcellar(ms1.labels,'H')),recnum);
distmatr = repmat(0,recnum,numel(ms1.prop.distmatr));
anglematr = repmat(0,recnum,numel(ms1.prop.anglematr));
tormatr = repmat(0,recnum,numel(ms1.prop.tormatr));
...
%    sdescnew(i) = {ms0.prop.sdescnew};
    if strcmp(usedpackage,'Gaussian')
      filename(i) = {ms0.filename};
      worktitle(i) = {ms0.worktitle};
    else
    ...
    talpha(i) = ms0.prop.alpha;
    tbeta(i) = ms0.prop.beta;
    tgamma(i) = ms0.prop.gamma;

    if isfield(ms0.prop,'delta');
        tdelta(i) = ms0.prop.delta;
    else
        tdelta(i) = NaN;
    end
    if isfield(ms0.prop,'epsilon');
        tepsilon(i) = ms0.prop.epsilon;
    else
        tepsilon(i) = NaN;
    end
    ...
    if ~( any(moltype==[7,8]) || (moltype>=900 && moltype<=999) )
      if isfield(ms0.prop,'tau01');
        tau01(i) = ms0.prop.tau01; %angle between C1'N1 and aminobase plane (previously aglycoutplane)
      end
      if isfield(ms0.prop,'maxtorinbase');
    ...
    if isfield(ms0.prop,'dplC2');
      dplC2(i) = ms0.prop.dplC2;
    end
    if isfield(ms0.prop,'dplC3');
...
if strcmp(usedpackage,'Gaussian') && 0

%[HbondLMfreq,HbondVMfreq]=deal(zeros(size(HbondLMind)));
   for i=1:recnum
     ind=zeros(0);
     if ~((moltype>100 && mod(moltype,100)==30) || moltype==22) %non 5'nucleotides
       iH53= workdb(i).ind(find(find(strcmp(pind.labels,'pH53'))==workdb(i).pind)); 
       ind(end+1)=find(Hind(i,:)==iH53);
     end
     if ~((moltype>100 && any(mod(moltype,100)==[31,12])) || moltype==23) %non 3'nucleotides or dideoxy molecules
...
%  r(p>0.1)=NaN;
  ...
  if mod(moltype,100)==11 %tionucleosides
      ...
      buf = strcmpcellar(tormatrdesc,'pC2pC1pO4pC4'); %nu0
      maintorinds(1)=buf(1);
      buf = strcmpcellar(tormatrdesc,'pC3pC2pC1pO4'); %nu1
      maintorinds(2)=buf(1);
      buf = strcmpcellar(tormatrdesc,'pC1pC2pC3pC4'); %nu2
      maintorinds(3)=buf(1);
      buf = strcmpcellar(tormatrdesc,'pC2pC3pC4pO4'); %nu3
      maintorinds(4)=buf(1);
      buf = strcmpcellar(tormatrdesc,'pC1pO4pC4pC3'); %nu4
      maintorinds(5)=buf(1);
  end
  ...
  torsigns(2)=-1;
  torsigns(5)=-1;
  buf = strcmpcellar(tormatrdesc,'pC3pC4pC5pO5'); %gamma
  maintorinds(6)=buf(1);
  torsigns(6)=-1;
  ...
  elseif ((moltype>100 && mod(moltype,100)==31) || moltype==23) %3'nucleotides
      ...
      buf = strcmpcellar(tormatrdesc,'pC4pC3pO3pH32'); %epsilon
      maintorinds(7)=buf(1);
  end
  ...
  if ((moltype>100 && mod(moltype,100)==30) || moltype==22) %5'nucleotides
      ...
      buf = strcmpcellar(tormatrdesc,'pC4pC5pO5pH53'); %beta
      maintorinds(8)=buf(1);
  end
  torsigns(8)=-1;
  ...
  elseif mod(moltype,100)==11 %tionucleosides
      ...
      buf = strcmpcellar(tormatrdesc,'pO4pC4pC3pO3'); %delta
      maintorinds(9)=buf(1);
  end
  ...
  elseif any(moltype==[7,8]) || (moltype>=900 && moltype<=999)
    %do nothing
  else
...
%---main torsions correlation - do only for syn conformers begin
  ...
  %ZPE=CC.ZPEcoefB3LYP631Gd*ZPE;
  %TEC=CC.ZPEcoefB3LYP631Gd*TEC;
  %EEC=EEC;
  %GEC=GEC;
  ...
  GOenergyZPEkm = dGOenergykm+GEC;
  SPenergyZPEkm = dSPenergykm+GEC;
  dGOenergyZPEkm = GOenergyZPEkm-min(GOenergyZPEkm);
  dSPenergyZPEkm = SPenergyZPEkm-min(SPenergyZPEkm);


  i=1;
  datacalc=[dGOenergykm; dSPenergykm; dGOenergyZPEkm; dSPenergyZPEkm];
  ...
  if exist('mostaccenergyfnind','var')
      ...
      %    gausenergy_ = remels(gausenergy,iexclude); %exclude beta-ribose conformations (150 degree is critical value)
          ...
          gausenergyZPEkm = gausenergykm+repmat(GEC,size(gausenergy,1),1);  %GEC correcting
          dgausenergyZPEkm = gausenergyZPEkm-repmat(min(gausenergyZPEkm,[],2),1,recnum); %GEC correcting

          xlswrite1spec(xlsfile,[dgausenergyZPEkm; gausenergykm(mostaccenergyfnind,:)]','ribose','EA3');
      end
  end
  ...
  for i=2:size(indHbond,1)
      calcHbondnum{i}=['=явер(' indHbond(i,1:2) int2str(3) ':' indHbond(i,1:2) int2str(2+recnum) ')']; %{'=явер(FA3:FA64)'}
  end
