...
%dftV3bis2 - b3lyp/6-31G(d,p) opt=(verytight) Int=UltraFine 2010-0201
%dftV4 - b3lyp/6-31G++(d,p)
%dftV5 - b3lyp/6-311G++(d,p)
%mp2V2 - mp2/6-31G(d,p)
%mp2V5 - mp2/6-311G++(d,p)
%mp2V6 - mp2/6-311G++(df,pd)
...
task.updateTS   = 0;      %true if files to process contains data on trasition state structures
task.updateFixed= 0;      %true if files to process contains data on trasition state structures ???
...
%T	transition state
%C  ???
%F  conformation with fixed parameters
%D  DNA like conformation
...
%    delete(gca);
    ...
    fseek(fid,-500,'eof'); 
    streof = fscanf(fid,'%s',inf); %last 500 chars of file
  ...
  %          end
         ...
         if any(moltype == [910,920,950])
             ms0 = calcproperties910(ms0,moltype,1);
         else
             ms0 = calcproperties(ms0,moltype,1);
         end
         ...
         %criteria of structure identity
         dx=0.0001;
...
%            indwork=0;
      ...
      if task.opt %analyse geometry optimization results
          ...
          while 1
            ...
            if ~isempty(strfind(tline,'Standard basis'))
              pat = 'Standard basis:\s+(\S+)\s+';
            ...
            if ~isempty(strfind(tline,'IExCor'))
              pat = '\sIExCor=\s*(\d+)[\s$]';
        	  A = regexp(tline, pat, 'tokens','once');
	          IExCor = sscanf(A{1},'%d');
              continue;
            ...
            if ~isempty(strfind(tline,'SCF Done'))
              [xxx,step_energy]=strread(tline,'%s%f','delimiter','=');
              continue;
            end
...
%          if (isDFT=='T' && strcmp(Ex,'B+HF') && strcmp(Corr,'LYP') && (ScaHFX==0.2) )
          if (IExCor==402)
          	theory = 'B3LYP';
          elseif (IExCor==205)
            disp([worktitle,': As of IExCor=' int2str(IExCor) ' assuming theory to be MP2. Please check if correct...']);
          	theory = 'MP2';
          else
            disp([worktitle,': Unknown theory with IExCor=' int2str(IExCor) ' detected. Please learn me...']);
            fclose(fid);
            continue
          end

          basis_string_unified = regexprep(basis_string, '+', '_');
          basis_string_unified = regexprep(basis_string_unified, '[-,()]+', '');

		  method = [theory '_' basis_string_unified];
          ...
          if ~exist('step_energy','var')
            disp([worktitle,': Error in file detected: no steps found']);
            fclose(fid);
            continue
          end
          ms0.GO.energy = step_energy;
          ms0.gaussian = setfield(ms0.gaussian, method, step_energy);
          ms0.gaussian.stpoint = isStPoint;
...
%              if ~isempty(strfind(tline,'Standard orientation')) %zhr090522
          ...
          if ~isfield(ms0.gaussian,'T') && (isfield(ms0.gaussian,'TEC') || isfield(ms0.gaussian,'EEC') || isfield(ms0.gaussian,'GEC'))
