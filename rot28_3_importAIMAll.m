...
%110511 added processing of .extout files 
%files are now differing based on their extentions
...
%filestype = 'mgp' %#ok % 'crt' or 'mgp' - obsolete
...
indir='E:\CAN2\data\r7\r7_dftV3bis\wfn' %#ok
%indir='/home/zhr/science/CAN2/data/r390/r39011_aimall'
%--------- data section end ---------------------------
...
sfiles = dir(strcat(indir, filesep, '*.mgp')); %zhr110511 2do: will be better to concat results 
if isempty(sfiles), sfiles = dir(strcat(indir,filesep, '*.crt'));, end
if isempty(sfiles), sfiles = dir(strcat(indir,filesep, '*.extout'));, end
...
%try
    ...
    %         tline = fgetl(fid);%DelSqRho
            ...
            pat = '\s+Bond\sEllipticity\s+=\s+(.*)';
%            -?\d*\.*\d+E?[+-]?\d* //removed zhr101124
            A = regexp(buf, pat, 'tokens','once');
            A=[A{:}];
    ...
    elseif strcmp(fext, 'crt') || strcmp(fext,'extout')
...
%end    
    ...
    for i=1:numel(CPs)
       ...
       if any(ms0.labels{atoms(1)}~='H') && any(ms0.labels{atoms(2)}~='H')
       ...
       elseif any(ms0.labels{atoms(1)}~='H')
       ...
       elseif any(ms0.labels{atoms(2)}~='H')
       ...
       if (~isfield(ms0,'nfrag')) %zhr110310
           HBatomsfrags = ones(size(HBatoms));
       else
           HBatomsfrags = ms0.nfrag(HBatoms);
       end
      
       HBatomsfrags(HBatomsfrags==0)=1;
...
%       AIM.BondEl(end+1)=CPs(i).BondEl;
       ...
       AIM.nfrag(end+1,1:numel(HBatomsfrags))=HBatomsfrags;
