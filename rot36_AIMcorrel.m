...
fl_selectHBtypes=1 %#ok %display data only for selected Hbond types
Atype='O' %#ok A atom type for AH...B bond
Btype='O' %#ok B atom type for AH...B bond
fl_plot_AHB_AB=0; % E_AHB_AB or E_rho_deltarho
fl_plotmnk=0;
%---------------------------------
...
for m_ind=1:numel(db)
    ...
    for i_rec=1:recnum
       ...
       for i_HB=1:HBnum %over H-bonds in molecule
...
if 0
    for iii=1:numel(db),db(iii).title, if isempty(db(iii).HB)==0, max([db(iii).HB.rho]), end, end

    mol_ind=7;
    max(db(mol_ind).HB.rho)
    db(mol_ind).HB.sdesc(find(db(mol_ind).HB.rho==max(db(mol_ind).HB.rho)))
end
