...
%    ipC2 = ms0.ind(find(find(strcmp(pind.labels,'pC2'))==ms0.pind)); 
    ...
    if ~any(moltype==413) %is not AZT
        ipO3 = ms0.ind(find(find(strcmp(pind.labels,'pO3'))==ms0.pind)); %#ok
        ipH32= ms0.ind(find(find(strcmp(pind.labels,'pH32'))==ms0.pind)); %#ok
    else %zhr101223
        ipO3 = ms0.ind(find(find(strcmp(pind.labels,'pN31'))==ms0.pind)); %#ok
        ipH32= ms0.ind(find(find(strcmp(pind.labels,'pN32'))==ms0.pind)); %#ok
    end
...
%rotates around tgamma 
    ...
    if ~any(opt)
       ...
       elseif ~isempty(ipH22) && ~isempty(ipH32) && adist(ms1,ipH22,ipH32)<minHHdist
...
%         savemolnw(odir,ms1,4); 
       ...
       display_skip_reason(desc,'gamma+120',opt,foundintrieddesc);
    end
    descstr=rotatesdesc(descstr,curind2rotate);
    [iprev,inext] = checksimdesc(descstr,desc);
    foundintrieddesc=strcmpcellar(trieddesc,descstr);
...
%         savemolnw(odir,ms1,4); 
        ...
        display_skip_reason(desc,'gamma+240',opt,foundintrieddesc);
    end
    descstr=rotatesdesc(descstr,curind2rotate);
...
%         savemolnw(odir,ms1,4); 
        ...
        display_skip_reason(desc,'beta+120',opt,foundintrieddesc);
    end
    descstr=rotatesdesc(descstr,curind2rotate);
    [iprev,inext] = checksimdesc(descstr,desc);
    foundintrieddesc=strcmpcellar(trieddesc,descstr);
...
%         savemolnw(odir,ms1,4); 
        ...
        display_skip_reason(desc,'beta+240',opt,foundintrieddesc);
    end
    descstr=rotatesdesc(descstr,curind2rotate);
    ...
    %         savemolnw(odir,ms1,4); 
            ...
            display_skip_reason(desc,'epsilon+120',opt,foundintrieddesc);
        end
        descstr=rotatesdesc(descstr,curind2rotate);
        [iprev,inext] = checksimdesc(descstr,desc);
        foundintrieddesc=strcmpcellar(trieddesc,descstr);
    ...
    %         savemolnw(odir,ms1,4); 
            ...
            display_skip_reason(desc,'epsilon+240',opt,foundintrieddesc);
        end
        descstr=rotatesdesc(descstr,curind2rotate);
    elseif i==1
        warning('Molecule detected as 3''-deoxy. Rotates round C3''O3'' are not performed.'); %#ok
    end
...
%rotates around teta
    ...
    %         savemolnw(odir,ms1,4); 
            ...
            display_skip_reason(desc,'eta+120',opt,foundintrieddesc);
        end
        descstr=rotatesdesc(descstr,curind2rotate);
        [iprev,inext] = checksimdesc(descstr,desc);
        foundintrieddesc=strcmpcellar(trieddesc,descstr);
    ...
    %         savemolnw(odir,ms1,4); 
            ...
            display_skip_reason(desc,'eta+240',opt,foundintrieddesc);
        end
        descstr=rotatesdesc(descstr,curind2rotate);
    elseif i==1
        warning('Molecule detected as 2''-deoxy. Rotates round C2''O2'' are not performed.'); %#ok
    end
    ...
    if any(moltype~=[7,8]) %nucleosides/nucleotides
     ...
     %         savemolnw(odir,ms1,4); 
            ...
            display_skip_reason(desc,'chi change',opt,foundintrieddesc);
        end
