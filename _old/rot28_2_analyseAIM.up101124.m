...
flwrite_bondlist=1 %#ok %output table with all CPs parameters
...
%       end
      ...
      if ~isempty(atom3ind)
          AIM.ABdist(curinds(j))=adist(ms0,atom1ind(1),atom3ind(1));
          AIM.HBdist(curinds(j))=adist(ms0,atom2ind(1),atom3ind(1));
          AIM.AHBang(curinds(j))=valang(ms0,atom1ind(1),atom2ind(1),atom3ind(1));
      else
...
if flwrite_bondlist %zhr091208
  ...
  str=sprintf('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s',...
              'sdesc','bond','rho','DelSqRho','V','G','K','L');
  disp(str);
  allHbondsdata = cell(0,8);
  ...
  for i=1:recnum
      ...
      for j=1:numel(ms0.AIM.desc)
        ...
        K         = ms0.AIM.K(j);
        L         = ms0.AIM.L(j);
        str=sprintf('%s\t%s\t%7.3f\t%7.3f\t%7.3g\t%7.3g\t%7.3g\t%7.3g',...
                    ms0.prop.sdesc,bondstr,ro,DelSqRho,V,G,K,L);
        disp(str);
        allHbondsdata(end+1,:) = [{ms0.prop.sdesc},{bondstr},{ro},{DelSqRho},{V},{G},{K},{L}];
