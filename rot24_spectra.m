...
if moltype==12 %diss
    molname='dUrd'
    workdbname=[CD.dbdir filesep 'r12_g_dftV3_or.mat'] 
elseif moltype==13 %diss
    molname='dThd'; 
    workdbname=[CD.dbdir filesep 'r13_g_dftV3_or.mat'] 
elseif moltype==140
    molname='rAdo'; 
...
elseif moltype==540 %diss
    molname='rUrd'; 
    workdbname=[CD.dbdir filesep 'r540_g_dftV3_or.mat'] 
else
...
%numconf2plot=[1:111];
%numconf2plot=[6 20 Inf];
numconf2plot=[Inf];

%conf2exclude = [{'BaacS'},{'BaaaS'}]; %these conformers will not be included in spectra
conf2exclude = [];

labelHval=[{'pH32'}]  %label of H atom of valence vibrational mode. If specified - only these line in spectra will be plotted in 1/4 plot
labelHval2=[{'pH53'}]  %label of H atom of valence vibrational mode. If specified - only these line in spectra will be plotted in 2/4 plot
fl_plotcurconf=1 %plot current conformer section
...
%-------------------------------------------4 
  ...
  %  plot(X,sumY,color(mod(j,numel(color))+1));
    ...
    if fl_plotgauss
      ...
      %hgauss4=plot(X,sumY(1,:)-sumY(2,:)-sumY(3,:),'k'); %plot difference between summary spectra and convolution sections
  ...
  %  confnumber=input('Conformation number?: ');
    ...
    title([molname ' ' int2str(T) 'K IR spectra: ' int2str(j) ' conformer(s) contribution'],'FontSize',18);
  %  pause
